...
const facultyTour:FacultyTourStop[] = [
  ...
  { title:"Scholarship that develops people",district:"Teaching, mentorship, and service",position:[0,-45,0],argument:"My academic contribution includes how I build research capacity. I teach through consequential systems, mentor toward intellectual ownership, and create pathways from internships and early research questions to peer-reviewed scholarship.",evidence:["More than fifteen students supervised across international internship cohorts","Cohort research matured into peer-reviewed publications in agriculture, sensing, and workforce development","Service across CVPR, ICML, NeurIPS, WACV, WCNC, and leading journals"],links:[{label:"Teaching experience",href:"/teaching"},{label:"Professional record",href:"/cv"}] },
  { title:"Why work with me?",district:"Narrative conclusion",position:[11,-61,0],argument:"I bring a coherent interdisciplinary agenda, evidence that I can build working research systems, a record of peer-reviewed scholarship, and a mentorship practice that turns participation into ownership. I connect computing with infrastructure, transportation, energy, agriculture, and human decision-making while helping collaborators and institutions expand research impact and student opportunity.",evidence:["Research independence demonstrated through connected systems and a forward-looking agenda","International collaboration across Nigeria, South Korea, and the United States","Experience translating theory into prototypes, demonstrations, publications, and student outcomes","A distinctive foundation for an interdisciplinary laboratory in trustworthy intelligent systems"],links:[{label:"Download and review my CV",href:"/cv"},{label:"Discuss collaboration",href:"/contact"}],final:true },
];
...
const forestStories: Exhibit[] = [
  ...
  { title:"Teaching philosophy", kind:"Learning through systems", category:"Teaching", description:"I teach by connecting mathematical foundations, computational experiments, and consequential physical-world problems.", href:"/teaching", image:"", position:[0,0,0] },
  { title:"Mentorship philosophy", kind:"Ownership, rigor, and confidence", category:"Mentorship", description:"I help emerging researchers move from guided participation toward intellectual ownership, publication, and independent judgment.", href:"/#mentorship", image:"/gallery/uw-digital-twins-meetup-3.jpg", position:[0,0,0] },
...
export default function ResearchWorldClient({conferencePapers}:{conferencePapers:ConferencePaper[]}) {
  ...
  useEffect(() => {
    ...
    const bridgeWater=new THREE.Mesh(new THREE.PlaneGeometry(18,13,10,8),new THREE.MeshPhysicalMaterial({color:0x3d8994,transparent:true,opacity:.82,roughness:.18,metalness:.08,side:THREE.DoubleSide}));bridgeWater.rotation.x=-Math.PI/2;bridgeWater.position.y=.04;bridgeDistrict.add(bridgeWater);
    [-8,8].forEach(x=>{const bank=new THREE.Mesh(new THREE.BoxGeometry(3.2,.55,13),new THREE.MeshStandardMaterial({color:0x526d48,roughness:1}));bank.position.set(x,.18,0);bridgeDistrict.add(bank);const abutment=new THREE.Mesh(new THREE.BoxGeometry(1.1,2.7,4.4),new THREE.MeshStandardMaterial({color:0x8b8880,roughness:.85}));abutment.position.set(x>0?6.6:-6.6,1.35,0);bridgeDistrict.add(abutment);});
    const deck=new THREE.Mesh(new THREE.BoxGeometry(14,.42,3.6),new THREE.MeshStandardMaterial({color:0x77746e,metalness:.18,roughness:.72}));deck.position.y=2.65;bridgeDistrict.add(deck);
    const roadway=new THREE.Mesh(new THREE.BoxGeometry(13.8,.12,2.75),new THREE.MeshStandardMaterial({color:0x2f3437,roughness:1}));roadway.position.y=2.91;bridgeDistrict.add(roadway);
    [-.82,.82].forEach(z=>{const laneLine=new THREE.Mesh(new THREE.BoxGeometry(13.2,.025,.07),new THREE.MeshBasicMaterial({color:0xf5eee2}));laneLine.position.set(0,2.99,z);bridgeDistrict.add(laneLine);});
    for(let x=-5.5;x<=5.5;x+=2.2){const centerLine=new THREE.Mesh(new THREE.BoxGeometry(1.1,.03,.08),new THREE.MeshBasicMaterial({color:0xe5b951}));centerLine.position.set(x,3,0);bridgeDistrict.add(centerLine);}
    [-1.72,1.72].forEach(z=>{const railTop=new THREE.Mesh(new THREE.BoxGeometry(14,.09,.09),new THREE.MeshStandardMaterial({color:0xb8b7b1,metalness:.7,roughness:.28}));railTop.position.set(0,3.55,z);bridgeDistrict.add(railTop);for(let x=-6.5;x<=6.5;x+=1.3){const railPost=new THREE.Mesh(new THREE.BoxGeometry(.07,.75,.07),new THREE.MeshStandardMaterial({color:0x92938f,metalness:.65}));railPost.position.set(x,3.22,z);bridgeDistrict.add(railPost);}});
    const concrete=new THREE.MeshStandardMaterial({color:0x8b8983,roughness:.88});
    [-4,4].forEach(x=>{const pier=new THREE.Mesh(new THREE.BoxGeometry(.85,2.55,2.5),concrete);pier.position.set(x,1.28,0);bridgeDistrict.add(pier);[-1.38,1.38].forEach(z=>{const towerLeg=new THREE.Mesh(new THREE.BoxGeometry(.5,4.6,.5),concrete);towerLeg.position.set(x,5.05,z);bridgeDistrict.add(towerLeg);});const towerBeam=new THREE.Mesh(new THREE.BoxGeometry(.68,.5,3.25),concrete);towerBeam.position.set(x,6.9,0);bridgeDistrict.add(towerBeam);});
    ...
    const campusData=[
      {name:"FUTO, Nigeria",detail:"BEng · 2014",description:"My Bachelor of Engineering in Petroleum Engineering was conferred by the Federal University of Technology, Owerri, Nigeria, on December 12, 2014.",model:"b",z:-8},
      {name:"Kumoh National Institute of Technology",detail:"MSc · 2021 | PhD · 2025",description:"At Kumoh National Institute of Technology in Gumi, South Korea, I completed an MSc in Electronics Engineering and a PhD in IT Convergence Engineering. My laboratory affiliations included FCSL, the ICT Convergence Research Center, and NSL.",model:"c",z:-22},
      {name:"Kyungpook National University",detail:"Research Specialist · 2022–2024",description:"As Research Specialist and Supervisor from August 2022 to July 2024, I coordinated international research cohorts spanning AI-enabled sensing, digital agriculture, explainable machine learning, and workforce development.",model:"d",z:-36},
      {name:"Michigan State University",detail:"Visiting Research Scholar · 2024",description:"My Michigan State University appointment and CLIMDES collaboration connected climate-smart decision support, student mentorship, digital agriculture, and international research collaboration.",model:"e",z:-50},
    ...
    campusData.forEach((campus,index)=>{
      const exhibit:Exhibit={title:campus.name,kind:campus.detail,category:"Milestone",description:campus.description,href:index===campusData.length-1?"/#bio":"/cv",image:"",position:[-30,0,campus.z]};
      ...
      const datePlaque=new THREE.Sprite(new THREE.SpriteMaterial({map:makeLabel(campus.detail,"#8ebbc4"),transparent:true}));datePlaque.position.set(6.4,4.25,0);datePlaque.scale.set(6.2,1.45,1);campusGroup.add(datePlaque);proximityLabels.push(datePlaque);
        ...
        <>
          ...
          {welcomeOpen && (
            <aside className={styles.welcomeSpeech} role="dialog" aria-labelledby="welcome-guide-title">
              ...
              <div className={styles.welcomeActions}>
                <button type="button" className={styles.facultyTourButton} onClick={startFacultyTour}>Start the Tour</button>
          ...
          {facultyTourStep !== null && (()=>{
            ...
            return <aside className={`${styles.facultyTourPanel} ${stop.final?styles.facultyTourFinal:""}`} aria-live="polite">
              <div className={styles.tourProgress}><span>Five-minute Tour</span><strong>{facultyTourStep+1} / {facultyTour.length}</strong></div>
              ...
              {stop.final&&<div className={styles.collaborationAnswer}><strong>The case for working together</strong><span>Research vision, system-building ability, interdisciplinary range, teaching commitment, mentorship evidence, and international collaboration are already connected in one academic program.</span></div>}
          ...
          {mapOpen && (
            <aside className={styles.mapPanel}>
              ...
              <div className={styles.destinationGrid} aria-label="Museum destinations">
                <button type="button" className={styles.facultyTourDestination} onClick={startFacultyTour}><strong>Start the Tour</strong><span>A five-minute evidence-based introduction to my work</span></button>
